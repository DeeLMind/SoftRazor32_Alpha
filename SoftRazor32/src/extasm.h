...
typedef struct t_asmmodel {            // Model to search for assembler command
  char           code[MAXCMDSIZE];     // 二进制代码
  char           mask[MAXCMDSIZE];     // Mask for binary code (0: bit ignored)
  int            length;               // 代码长度, bytes (0: empty)
  int            jmpsize;              // 偏移量的大小,如果相对跳转
...
extern "C"
{
  ...
  unique int __stdcall Assemble(char *cmd, ulong ip, t_asmmodel *model, int attempt, int constsize, char *errtext);
  unique int __stdcall Isfilling(ulong addr, char *data, ulong size, ulong align);
